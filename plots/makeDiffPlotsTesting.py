...
dhist = dataFile.Get("diffP26").Clone("dclone")
shist = simFile.Get("diffP26").Clone("dclone")
...
dDhist.Scale(1./np.max(np.abs([dDhist.GetMaximum(), dDhist.GetMinimum()])))
...
ufftBasis = np.resize(ufftBasis, [NfftBasis, -1])
lfftBasis = np.fromfile("../rotorBasis/lclsfftRotorbasis.dat", dtype = np.double)
lfftBasis = np.resize(lfftBasis, [NfftBasis, -1])
...
tmp = np.fromfile("../simulation/rotation/output/expVals/expValCos2_315.914000-320.214000-0.010000.dat", dtype = np.double)
uCosEV = np.zeros((1,tmp.shape[0]), dtype = np.double)
uCosEV[0,:] = tmp
